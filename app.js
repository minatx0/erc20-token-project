...
const myTokenABI = []; // Fill your token's ABI here
const myTokenAddress = ''; // Fill your token's contract address here
...
async function transferTokens(toAddress, amount) {
  try {
    ...
    const txObject = {
      from: myAccount,
      to: myTokenAddress,
      data: myToken.methods.transfer(toAddress, web3.utils.toWei(amount, 'ether')).encodeABI(), // Adjust for token decimals if necessary
      gasPrice: gasPrice,
      gas: 2000000,
      nonce: nonce,
    };
...
function setupEventListeners() {
  myToken.events.Transfer({
    ...
    if (error) console.error('Transfer event error:', error);
    else {
